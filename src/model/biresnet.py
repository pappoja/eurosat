...
class BiResNet50(nn.Module):
    def __init__(self, num_classes, num_non_image_features, num_countries, input_type, embedding_dim=16):
        ...
        elif input_type == 'image_country_all':
            self.fc_combined = nn.Linear(self.num_resnet_features + embedding_dim + 64, num_classes)
        else:
            raise ValueError(f"Unknown input type: {input_type}. Must be one of: 'image', 'image_country', 'image_country_all'")
